...
uniform vec3 R;              // ellipsoid radius
uniform vec3 L;  // light source
...
float l;
vec4 draw_obj( vec4 fc, vec3 O, mat3 M, vec2 pos, int mode) {
	vec3 D = normalize(M*vec3(1.,pos));		// ray
	
	vec3 P,N; 
	if (! intersect_ellipsoid(O,D, P,N,l)) return fc;
...
vec4 mainImage(  vec4 fragColor,  vec2 fragCoord ) {
	...
	fragColor = draw_obj(fragColor, O,M, 1.5*(uv+dx), 1);	
	fragColor = draw_obj(fragColor, O,M, 1.5*(uv-dx), 1);	
...
void main() {
	...
	vec2 uv = vUv;
	gl_FragColor = mainImage( vec4( 0.0, 0.0, 0.0, 0.0 ), (uv) ) * opacity;
